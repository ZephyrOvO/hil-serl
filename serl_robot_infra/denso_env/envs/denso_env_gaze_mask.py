...
import numpy as np  # already imported above, kept for clarity
from serl_launcher.networks import reward_classifier_gaze as gaze_net
import threading
...
class ImageDisplayer(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            # )
            pref_keys = ["tactile_thumb", "tactile_index", "tactile_middle"]
            show_list = []
            if all(k in img_array and isinstance(img_array[k], np.ndarray) for k in pref_keys):
                for k in pref_keys:
                    v = img_array[k]
                    if v.ndim == 2:
                        v = cv2.cvtColor(v, cv2.COLOR_GRAY2BGR)
                    show_list.append(cv2.resize(v, (128, 128)))
            else:
                show_list = [cv2.resize(v, (128, 128)) for k, v in img_array.items() if "full" not in k]
            if len(show_list) == 0:
                show_list = [np.zeros((128,128,3), dtype=np.uint8)]
            frame = np.concatenate(show_list, axis=1)
...
class DensoEnv(gym.Env):
    def __init__(
        ...
        if self.enable_tactile and not self.fake_env:
            ...
            # 1) 拿到中指 ref（Sensor 在 calibrated=True 时会准备好 self.ref）
            self.middle_ref_bgr = self.middle_tactile_sensor.ref.copy()
            _ref_gray = cv2.cvtColor(self.middle_ref_bgr, cv2.COLOR_BGR2GRAY)
            _hm0 = self.middle_tactile_sensor.raw_image_2_height_map(_ref_gray)
            _hm0 = self.middle_tactile_sensor.expand_image(_hm0)
            _hm0_u8 = cv2.normalize(_hm0, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
            _hm0_jet = cv2.applyColorMap(_hm0_u8, cv2.COLORMAP_JET)
            self.middle_ref_heatmap = cv2.resize(_hm0_jet, (128, 128), interpolation=cv2.INTER_LINEAR)
            self.lock_middle_enable = True
            self.lock_middle_mode = "heatmap"
        ...
        if self.display_image:
            ...
            # self.displayer = ImageDisplayer(self.img_queue, name=f"{self.url}-grid")
            # self.displayer.start()
        ...
        self.frame_save_path = "/mnt/data3/hcj/recorded_data_robot_new/recorded_data_training-10-11-try"  # 可自行修改
        ...
        # ===== Gaze classifier & SAM2 (single-frame prompt) =====
        self.gaze_ckpt = "/home/ruiqiang/workspaces/HK_TacExo_HAN/hil-serl/examples/gaze_ckpt/checkpoint_100"
        self._rng = jax.random.PRNGKey(0)
        # 建一个与训练期一致的 dummy sample 以正确 init 参数结构（单张 640x480 RGB）
        _H0, _W0 = 480, 640
        _dummy = {"images": {"front_camera": jnp.zeros((1, _H0, _W0, 3), jnp.float32)}}
        self.gaze_apply = gaze_net.load_gaze_bbox_classifier_func(
            self._rng, _dummy, ["front_camera"], checkpoint_path=self.gaze_ckpt
        )
        # SAM2 仅做单帧 box→mask
        self.sam2_single = self._load_sam2_predictor("facebook/sam2-hiera-small")
        self._last_gaze_mask_full = None
    ...
    # --- Gaze: 前向推理，取 (pred_cls, norm_xyxy[4]) ---
    def _infer_gaze_box_from_rgb(self, bgr_img: np.ndarray) -> tuple[int, np.ndarray]:
        rgb = bgr_img[..., ::-1]  # BGR→RGB
        H0, W0 = 480, 640
        inp = cv2.resize(rgb, (W0, H0), interpolation=cv2.INTER_LINEAR)
        obs = {"images": {"front_camera": jnp.asarray(inp[None, ...] / 255.0, dtype=jnp.float32)}}
        # logits, gaze_box = self.gaze_apply(obs)
        out = self.gaze_apply(obs)
        if isinstance(out, tuple) and len(out) == 2:
            logits, gaze_box = out
        elif isinstance(out, dict) and "logits" in out and "gaze_box" in out:
            logits, gaze_box = out["logits"], out["gaze_box"]
            print("[gaze_bbox] WARNING: received dict; consider updating to tuple API.")
        else:
            raise TypeError(f"Unexpected gaze_apply() return type: {type(out)}")
        logits_np = np.array(logits[0])
        box_np    = np.clip(np.array(np.asarray(gaze_box)[0]), 0.0, 1.0)  # (4,) xyxy in [0,1]
        pred = int(np.argmax(logits_np).item())
        return pred, box_np
    
    def _safe_gaze_mask_hw(self, default_hw=(128, 128)) -> tuple[int, int]:
        """
        从 observation_space 推断 gaze_mask 的 (H, W)，失败则回退到 default_hw。
        永远返回正整数。
        """
        Hm, Wm = map(int, default_hw)
        try:
            sp = getattr(self, "observation_space", None)
            if sp is not None and hasattr(sp, "spaces"):
                # images 子字典优先
                imgs = sp.spaces.get("images", None)
                if imgs is not None and hasattr(imgs, "spaces"):
                    gm = imgs.spaces.get("gaze_mask", None)
                    if gm is not None and hasattr(gm, "shape"):
                        h, w = int(gm.shape[0]), int(gm.shape[1])
                        if h > 0 and w > 0:
                            return h, w
                # 扁平键
                gm = sp.spaces.get("gaze_mask", None)
                if gm is not None and hasattr(gm, "shape"):
                    h, w = int(gm.shape[0]), int(gm.shape[1])
                    if h > 0 and w > 0:
                        return h, w
            ...
            pass
        # 兜底 & 保证正值
        Hm = 128 if Hm <= 0 else Hm
        Wm = 128 if Wm <= 0 else Wm
        return Hm, Wm
    
    
    # --- Gaze: 用 SAM2 单帧 box-prompt 产 mask（uint8, 0/255, full-res） ---
    def _mask_from_box_sam2(self, bgr_img: np.ndarray, box_norm_xyxy: np.ndarray) -> np.ndarray:
        # --- 准备坐标 ---
        rgb = bgr_img[..., ::-1]
        H, W = rgb.shape[:2]
        x0 = int(round(float(box_norm_xyxy[0]) * W))
        y0 = int(round(float(box_norm_xyxy[1]) * H))
        x1 = int(round(float(box_norm_xyxy[2]) * W))
        y1 = int(round(float(box_norm_xyxy[3]) * H))
        # 边界裁剪 / 纠正顺序
        ...
        import numpy as _np
        import cv2, tempfile, os, shutil
        mask_full = None

        # === 关键修复：SAM2 目前只接受 MP4 或 JPEG 文件夹，这里把单帧写入临时 JPEG 目录后再 init_state ===
        tmp_root = tempfile.mkdtemp(prefix="sam2_oneframe_")
        frames_dir = os.path.join(tmp_root, "frames")
        os.makedirs(frames_dir, exist_ok=True)
        frame_path = os.path.join(frames_dir, "000000.jpg")
        # 写入 BGR JPEG（质量 95）
        cv2.imwrite(frame_path, bgr_img, [int(cv2.IMWRITE_JPEG_QUALITY), 95])

        try:
            with torch.inference_mode():
                state = self.sam2_single.init_state(
                    frames_dir,
                    offload_video_to_cpu=True,
                    offload_state_to_cpu=True,
                )
                self.sam2_single.add_new_points_or_box(
                    state, frame_idx=0, obj_id=0, points=None, labels=None,
                    box=_np.array([x0, y0, x1, y1], dtype=_np.float32),
                )
                # 兼容不同返回类型（有的版本返回生成器三元组，有的返回字典）
                out = self.sam2_single.propagate_in_video(state)
                if hasattr(out, "__iter__") and not isinstance(out, dict):
                    # 生成器/迭代器：取首帧
                    for fidx, obj_ids, masks in out:
                        m = masks[0]
                        try:
                            m = m.detach().cpu().numpy()
                        except Exception:
                            m = _np.asarray(m)
                        mask_full = (m > 0).astype(_np.uint8) * 255
                        break
                else:
                    # 字典形式：取 masks[0]
                    m = out["masks"][0]
        ...
        # return mask_full
        # --- 强制合法化：2D, uint8, 与原图同尺寸 ---
        if mask_full is None:
            mask_full = np.zeros((H, W), dtype=np.uint8)
        else:
            import numpy as _np
            mask_full = _np.squeeze(mask_full)
            # 允许 model 返回 3 通道概率图：转灰
            if mask_full.ndim == 3:
                if mask_full.shape[2] == 3:
                    mask_full = cv2.cvtColor(mask_full, cv2.COLOR_BGR2GRAY)
                elif mask_full.shape[2] == 4:
                    mask_full = cv2.cvtColor(mask_full, cv2.COLOR_BGRA2GRAY)
                elif mask_full.shape[2] == 1:
                    mask_full = mask_full[..., 0]
                else:
                    mask_full = mask_full[..., 0]
            
            if mask_full.ndim != 2 or mask_full.shape != (H, W):
                mask_full = cv2.resize(mask_full.astype(_np.uint8, copy=False), (W, H), interpolation=cv2.INTER_NEAREST)
            if mask_full.dtype != _np.uint8:
                mask_full = mask_full.astype(_np.float32, copy=False)
            if mask_full.max() <= 1.0:
                mask_full = (mask_full > 0.5).astype(_np.uint8) * 255
            else:
                mask_full = mask_full.astype(_np.uint8, copy=False)
        return mask_full
    ...
    def get_im(self) -> Dict[str, np.ndarray]:
        ...
        #             self._safe_put(self.img_queue, display_images)
        #     except Exception:
        #         pass
            ...
            try:
                if hasattr(self, "img_queue"):
                    with self.tac_thumb_lock:
                        thumb = copy.deepcopy(self.thumb_heat_map) if isinstance(self.thumb_heat_map, np.ndarray) else None
                    with self.tac_index_lock:
                        indexf = copy.deepcopy(self.index_heat_map) if isinstance(self.index_heat_map, np.ndarray) else None
                    with self.tac_middle_lock:
                        middle = copy.deepcopy(self.middle_heat_map) if isinstance(self.middle_heat_map, np.ndarray) else None
                    
                    disp = {}
                    if thumb is not None:   disp["tactile_thumb"] = thumb
                    if indexf is not None:  disp["tactile_index"] = indexf
                    if middle is not None:  disp["tactile_middle"] = middle
                    if len(disp) == 0:
                        dummy = np.zeros((128,128,3), dtype=np.uint8)
                        disp = {"tactile_thumb": dummy, "tactile_index": dummy, "tactile_middle": dummy}
                    self._safe_put(self.img_queue, disp)
            except Exception:
                pass
        ...
        if "gaze_mask" in self.observation_space["images"].spaces:
            # 优先用 front_camera 的 full-res（未 resize 的裁剪结果）
            src_full = display_images.get("front_camera_full", None)
            if src_full is None:
                src_full = display_images.get("front_camera", None)
            if src_full is not None:
                pred_cls, box_norm = self._infer_gaze_box_from_rgb(src_full)
                if pred_cls == 2:
                    mask_full = np.zeros(src_full.shape[:2], np.uint8)
                else:
                    mask_full = self._mask_from_box_sam2(src_full, box_norm)
                # self._last_gaze_mask_full = mask_full  # 缓存 full-res，供落盘
                Hsrc, Wsrc = src_full.shape[:2]
                if mask_full is None or mask_full.size == 0:
                    mask_full = np.zeros((Hsrc, Wsrc), dtype=np.uint8)
                ...
                if mask_full.ndim == 3:
                    if mask_full.shape[2] == 3:
                        ...
                        mask_full = mask_full[..., 0]
                if mask_full.shape != (Hsrc, Wsrc):
                    mask_full = cv2.resize(mask_full, (Wsrc, Hsrc), interpolation=cv2.INTER_NEAREST)
                if mask_full.dtype != np.uint8:
                    mask_full = mask_full.astype(np.uint8, copy=False)
                mask_full = np.ascontiguousarray(mask_full)
                self._last_gaze_mask_full = mask_full
                # 下采样到 obs 所需尺寸（3ch）
                Hm, Wm, Cm = self.observation_space["images"]["gaze_mask"].shape
                # mask_small = cv2.resize(mask_full, (Wm, Hm), interpolation=cv2.INTER_NEAREST)
                # images["gaze_mask"] = np.repeat(mask_small[..., None], 3, axis=2)
                
                
                # --- 统一保证 mask_full 是 2D uint8 ---
                Hsrc, Wsrc = src_full.shape[:2]
                if mask_full is None:
                    mask_full = np.zeros((Hsrc, Wsrc), dtype=np.uint8)
                else:
                    # 先 squeeze 一下，处理 (1,H,W)/(H,W,1) 这种外壳
                    mask_full = np.squeeze(mask_full)
                    if mask_full.ndim == 3:
                        Hm, Wm, Cm = mask_full.shape  # 可能是 HWC，也可能 squeeze 后仍是 CHW(例如 C,H,W 但 C!=1 时不会被 squeeze)
                        if Cm in (1, 3, 4) and (Hm, Wm) == (Hsrc, Wsrc):
                            if Cm == 3:
                                mask_full = cv2.cvtColor(mask_full, cv2.COLOR_BGR2GRAY)
                            elif Cm == 4:
                                mask_full = cv2.cvtColor(mask_full, cv2.COLOR_BGRA2GRAY)
                                ...
                                mask_full = mask_full[..., 0]
                        elif Hm in (1, 3, 4) and (Wm, Cm) == (Hsrc, Wsrc):
                            C = Hm
                            chw = mask_full
                            if C == 1:
                                mask_full = chw[0]
                            elif C == 3:
                                mask_full = cv2.cvtColor(np.transpose(chw, (1, 2, 0)), cv2.COLOR_BGR2GRAY)
                            elif C == 4:
                                mask_full = cv2.cvtColor(np.transpose(chw, (1, 2, 0)), cv2.COLOR_BGRA2GRAY)
                        else:
                            mask_full = mask_full.reshape(Hsrc, Wsrc)
                    elif mask_full.ndim == 2:
                        if mask_full.shape != (Hsrc, Wsrc):
                            mask_full = cv2.resize(mask_full, (Wsrc, Hsrc), interpolation=cv2.INTER_NEAREST)
                    else:
                        mask_full = np.asarray(mask_full).reshape(Hsrc, Wsrc)
                    if mask_full.dtype != np.uint8:
                        mask_full = mask_full.astype(np.uint8, copy=False)

                

                Hm, Wm = self._safe_gaze_mask_hw(default_hw=(128, 128))
                # mask_small = cv2.resize(mask_full, (int(Wm), int(Hm)), interpolation=cv2.INTER_NEAREST)
                mask_small = cv2.resize(mask_full, (int(Wm), int(Hm)), interpolation=cv2.INTER_NEAREST)
                images["gaze_mask"] = np.repeat(mask_small[..., None], 3, axis=2)
                
            else:
                Hm, Wm, Cm = self.observation_space["images"]["gaze_mask"].shape
                images["gaze_mask"] = np.zeros((Hm, Wm, Cm), dtype=np.uint8)
                self._last_gaze_mask_full = None
    ...
    def process_middle_tactile(self):
        ...
        #     time.sleep(0.01)
        # Process middle tactile data（支持“锁死”）
        while True:
            if getattr(self, "lock_middle_enable", False):
                with self.tac_middle_lock:
                    mode = getattr(self, "lock_middle_mode", "heatmap")
                    if mode == "heatmap":
                        self.middle_heat_map = self.middle_ref_heatmap.copy()
                    elif mode == "ref_bgr":
                        self.middle_heat_map = cv2.resize(self.middle_ref_bgr, (128, 128), interpolation=cv2.INTER_LINEAR)
                    elif mode == "zero":
                        self.middle_heat_map = np.zeros((128, 128, 3), dtype=np.uint8)
                    else:
                        self.middle_heat_map = self.middle_ref_heatmap.copy()
                time.sleep(0.01)
                continue
            middle_raw_img, middle_points, middle_heat_map = self.process_tactile_data(
                self.middle_tactile_sensor, (128, 128)
            )
            with self.tac_middle_lock:
                self.middle_raw_img = middle_raw_img
                self.middle_points = middle_points
    ...
    def close_cameras(self):
        ...
        try:
            ...
            #     cap.close()
            caps = getattr(self, "cap", None)
            if not caps:
                return
            for _c in list(caps.values()):
                try:
                    _c.close()
                except Exception:
                    pass
    ...
    def _send_leap_hand_command(self, leap_hand_action: np.ndarray, steps=10, step_time=0.01):
        ...
        self.curr_leap_hand_pos = np.asarray(current_pos, dtype=np.float64).copy()
    ...
    def leap_interpolate_and_publish(self, start_position, end_position, step_time, steps):
        for i in range(steps + 1):
            ...
            self.curr_leap_hand_pos = np.asarray(interpolated_position, dtype=np.float64).copy()
    ...
    def close(self):
        ...
        if self.display_image:
            ...
            # if hasattr(self, "img_queue"):
            #     try:
            #         self._safe_put(self.img_queue, None)
            #     except Exception:
            #         pass
            ...
            # except Exception:
            #     pass
            q = getattr(self, "img_queue", None)
            if q is not None:
                try:
                    q.put(None)
                except Exception:
                    pass
            disp = getattr(self, "displayer", None)
            if disp is not None:
                try:
                    disp.join()
                except Exception:
    ...
    def save_training_frame(self):
        ...
        try:
            ...
            # 写 gaze_mask.png（与 rs_mirror 同分辨率；若无则写零掩码）
            if self._last_gaze_mask_full is not None:
                cv2.imwrite(os.path.join(fdir, "gaze_mask.png"), self._last_gaze_mask_full)
            else:
                if "front_camera" in images:
                    Hm, Wm = images["front_camera"].shape[:2]
                else:
                    Hm, Wm = 480, 640
                cv2.imwrite(os.path.join(fdir, "gaze_mask.png"), np.zeros((Hm, Wm), np.uint8))
