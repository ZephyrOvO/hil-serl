...
flags.DEFINE_integer("resume_step", -1, "从指定的 checkpoint_xxx 恢复；-1 表示使用最新的 checkpoint")
...
def learner(rng, agent, replay_buffer, demo_buffer, wandb_logger=None):
    ...
    """
    from flax.core.frozen_dict import FrozenDict as _FD, freeze as _freeze, unfreeze as _unfreeze
    ...
    # demo_ratio = 0.7
    # replay_ratio = 0.3
    # demo_bs = int(config.batch_size * demo_ratio)
    # replay_bs = config.batch_size - demo_bs
    ...
    # replay_iterator = demo_buffer.get_iterator(
    #     sample_args={
    ...
    # # === REPLACE B1: batch canonicalizer (pure / non-inplace) ===
    # def _canon_obs(obs, image_keys=tuple(config.image_keys)):
    #     is_frozen = isinstance(obs, _FD)
    #     o = _unfreeze(obs) if is_frozen else (_copy.deepcopy(obs) if not isinstance(obs, dict) else dict(obs))

    #     imgs = o.get("images")
    #     if imgs is None:
    #         imgs = {}
    #         o["images"] = imgs
    #     elif isinstance(imgs, _FD):
    #         imgs = _unfreeze(imgs)
    #         o["images"] = imgs

    #     for k in image_keys:
    #         if k in o:
    #             imgs[k] = o[k]          # 复制到 images，但不 pop 顶层
    #     return _freeze(o) if is_frozen else o

    # def _canon_batch(batch):
    #     is_frozen = isinstance(batch, _FD)
    #     b = _unfreeze(batch) if is_frozen else (dict(batch) if isinstance(batch, dict) else _copy.deepcopy(batch))
    #     if "observations" in b and b["observations"] is not None:
    #         b["observations"] = _canon_obs(b["observations"])
    #     if "next_observations" in b and b["next_observations"] is not None:
    #         b["next_observations"] = _canon_obs(b["next_observations"])
    #     return _freeze(b) if is_frozen else b
    # # === END REPLACE B1 ===
    # base_aug = agent.config["augmentation_function"]
    
    
    # def aug_wrap(batch, rng):
    #     b = base_aug(batch, rng)
    #     b = _canon_batch(b) 
    #     return b
    # agent.config["augmentation_function"] = aug_wrap
    ...
    for step in tqdm.tqdm(
        ...
        for critic_step in range(config.cta_ratio - 1):
            ...
            # batch = _canon_batch(batch)
            # if step == start_step:
            #     try:
            #         o = batch["observations"]
            #         from flax.core.frozen_dict import FrozenDict
            #         o_unf = unfreeze(o) if isinstance(o, FrozenDict) else o
            #         imgs = o_unf.get("images", {})
            #         print("[DEBUG] top-level keys:", list(o_unf.keys()))
            #         print("[DEBUG] images keys:", list(imgs.keys()) if isinstance(imgs, dict) else "N/A")
            #     except Exception as e:
            #         print("[DEBUG] key print failed:", repr(e))
...
def main(_):
    ...
    agent_pick = jax.device_put(
        jax.tree.map(jnp.array, agent_pick), sharding.replicate()
    )
