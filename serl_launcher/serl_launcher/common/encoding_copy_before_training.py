...
class EncodingWrapper(nn.Module):
    ...
    def __call__(
        ...
        for image_key in self.image_keys:
            if isinstance(observations, dict) and image_key in observations:
                image = observations[image_key]
            else:
                images_dict = observations.get("images") if isinstance(observations, dict) else None
                if isinstance(images_dict, dict) and image_key in images_dict:
                    image = images_dict[image_key]
                else:
                    avail_top = list(observations.keys()) if isinstance(observations, dict) else "N/A"
                    avail_img = list(images_dict.keys()) if isinstance(images_dict, dict) else "N/A"
                    raise KeyError(
