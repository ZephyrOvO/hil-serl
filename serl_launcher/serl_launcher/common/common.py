...
class JaxRLTrainState(struct.PyTreeNode):
    ...
    def apply_loss_fns(
        ...
        if has_aux:
            grads = jtu.tree_map(lambda _, x: x[0], loss_fns, grads_and_aux)
            aux = jtu.tree_map(lambda _, x: x[1], loss_fns, grads_and_aux)
