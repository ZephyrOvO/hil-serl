...
class DensoEnv(gym.Env):
    ...
    def end_episode_and_collect(self):
        if self._cur_ep_start is None:
            return None
        end_idx = self.global_frame_id - 1
        if end_idx >= self._cur_ep_start:
            rng = (int(self._cur_ep_start), int(end_idx))
            self.episode_frame_ranges.append(rng)
        else:
            rng = None
        self._cur_ep_start = None
        print(f"[DensoEnv][end_ep] start={self._cur_ep_start} end={end_idx} -> rng={rng}")
        return rng
    ...
    def _load_sam2_predictor(self, default_repo: str):
        ...
        return SAM2VideoPredictor.from_pretrained(use_id)
    ...
    def _sam2_propagate_and_save(self, mode: str, mirror_dir: str, frame_root: str, selected_indices: list[int], save_union: bool=True):
        ...
        predictor = self._load_sam2_predictor("facebook/sam2-hiera-base-plus")
    ...
    def _build_gaze_contacts_subset(self, frame_root: str, selected_indices: list[int], et_to_rs_map: dict[int,int], flip_y=True, eye_wh=(1280,720)):
        ...
        def _read_mask(frame_dir: pathlib.Path, oid: int):
            for pref in ET_MASK_PREFIXES:
                ...
                if p.exists():
                    ...
                    if m is not None: return (m>0).astype(np.uint8)
            return None
        def _parse_gaze(frame_dir: pathlib.Path):
            ...
            if isinstance(d, dict):
                ...
                if "norm_pos" in d:
                    ...
                    return u,v,ts
            return None
        def _choose_hit(et_masks: dict, u: int, v: int):
            cands=[]
            for oid,m in et_masks.items():
                H,W=m.shape[:2]
                if 0<=u<W and 0<=v<H and m[v,u]>0:
                    cands.append(oid)
            if not cands: return None
            if len(cands)==1: return cands[0]
            best_oid,best_depth=None,-1
            for oid in cands:
                dist=cv2.distanceTransform(et_masks[oid].astype(np.uint8), cv2.DIST_L2, 3)
                depth=float(dist[v,u])
                if depth>best_depth: best_depth=depth; best_oid=oid
            return best_oid
        ...
        for idx in selected_indices:
            ...
            if u is not None and v is not None and et_masks:
                et_oid=_choose_hit(et_masks, u, v)
    ...
    # === NEW: 对外一键批处理（收满 x 次数据后从 actor 调用） ===
    # def run_inline_sam2_and_label(self, frame_ranges: List[Tuple[int,int]], y_prompts: int = 20):
    #     # 1) 关键帧选择 + 交互标注（只在 ET 侧）
    #     key_indices = self._select_keyframes(frame_ranges, y_total=y_prompts, min_per_ep=2, max_per_ep=5, include_ends=True, prefer_gaze=False)
    #     if key_indices:
    #         et_files = sorted([str(p) for p in pathlib.Path(self.et_mirror_dir).glob("*.jpg")], key=lambda x: int(pathlib.Path(x).stem))
    #         if et_files:
    #             self._sam2_annotate_keyframes(et_files, key_indices)
    #         else:
    #             print("[SAM2] et_images empty; skip prompt.")
    #     else:
    #         print("[SAM2] no keyframes selected; continue without prompts.")

    #     # 2) 传播（ET/RS 两侧，覆盖这批范围内所有帧）
    #     selected=[]
    #     for s,e in frame_ranges:
    #         s,e=int(s),int(e)
    #         if e<s: s,e=e,s
    ...
    #     if selected:
    #         self._sam2_propagate_and_save("et", self.et_mirror_dir, self.frame_root, selected_indices=selected, save_union=True)
    #         self._sam2_propagate_and_save("rs", self.rs_mirror_dir, self.frame_root, selected_indices=selected, save_union=True)
    #         # 3) 构建 gaze_contact.json（与离线一致）
    #         self._build_gaze_contacts_subset(self.frame_root, selected_indices=selected, et_to_rs_map={0:0,1:1}, flip_y=True, eye_wh=(1280,720))
    #         print(f"[SAM2] done. episodes={len(frame_ranges)} frames={len(selected)} prompts={len(key_indices)}")
    #     else:
    #         print("[SAM2] selected frame list empty; skip.")

    # === NEW: 给上层读取标签（frame_idx -> class_id/None） ===
    ...
    """ def _sam2_annotate_keyframes_generic(self, mirror_dir: str, key_indices: list[int], mode: str = "et"):
        ...
        while True:
            ...
            msg = f"[{mode}] key {cur_i+1}/{len(key_indices)} frame={kf} obj={self._sam2_active_obj_v2}  (j/k:prev/next, n:new obj, Enter:finish)"
            cv2.putText(vis, msg, (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,0), 2)
